...
template sudoku() {
    ...
    signal input answer[9][9]; // 0 where original puzzle is not blank
    signal output out;


    component mul = matElemMul(9,9);

    component unsolved[9][9];
    component solved[9][9];


    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            unsolved[i][j] = RangeProof(32);
              unsolved[i][j].in <== puzzle[i][j];
             unsolved[i][j].range[0] <== 0;
             unsolved[i][j].range[1] <== 9;
            assert(unsolved[i][j].out);

             // for solved
             solved[i][j] = RangeProof(32);
            solved[i][j].in <== answer[i][j] ;
            solved[i][j].range[0]<== 0;
            solved[i][j].range[1] <==9;
          assert(solved[i][j].out);

            mul.a[i][j] <== puzzle[i][j];
            mul.b[i][j] <== answer[i][j];
        }
    }
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
